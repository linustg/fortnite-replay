...
bool AesDecryptor::has_key() const { return m_impl->key_set; }
...
CryptoResult<void> AesDecryptor::decrypt_inplace(std::span<uint8_t> data) {
  ...
  // Disable padding - Unreal Engine replay data is already block-aligned
  ...
  // Decrypt the data
  if (!EVP_DecryptUpdate(m_impl->ctx, data.data(), &out_len, data.data(),
...
AesDecryptor::decrypt(std::span<const uint8_t> encrypted_data) {
...
CryptoResult<std::vector<uint8_t>>
AesDecryptor::decrypt(std::span<const uint8_t> key,
...
using OodleLZ_Decompress_Func = int64_t (*)(
    const uint8_t *src_buf, int64_t src_len, uint8_t *dst_buf, int64_t dst_len,
    int fuzz, int crc, int verbose, uint8_t *dst_base, int64_t e, void *cb,
    void *cb_ctx, void *scratch, int64_t scratch_size, int threadPhase);
...
struct OodleDecompressor::Impl {
  ...
  std::vector<std::string> get_search_paths() const {
    ...
    paths.push_back("C:\\Program Files\\Epic
    Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\oo2core_9_win64.dll");
    paths.push_back(
        "D:\\EpicGames\\Fortnite\\FortniteGame\\Binaries\\Win64\\oo2core_5_"
        "win64.dll");
#elif __APPLE__
    // macOS paths
...
OodleDecompressor &
OodleDecompressor::operator=(OodleDecompressor &&) noexcept = default;
...
CryptoResult<std::vector<uint8_t>>
OodleDecompressor::decompress(std::span<const uint8_t> compressed_data,
...
CryptoResult<size_t>
OodleDecompressor::decompress_to(std::span<const uint8_t> compressed_data,
  ...
  int64_t result = m_impl->decompress_func(
      compressed_data.data(), static_cast<int64_t>(compressed_data.size()),
      output.data(), static_cast<int64_t>(decompressed_size),
      0,       // fuzz
      0,       // crc
      0,       // verbose
      nullptr, // dst_base
...
ReplayDataProcessor::ReplayDataProcessor() : m_impl(std::make_unique<Impl>()) {}

ReplayDataProcessor::~ReplayDataProcessor() = default;

ReplayDataProcessor::ReplayDataProcessor(ReplayDataProcessor &&) noexcept =
    default;
ReplayDataProcessor &
ReplayDataProcessor::operator=(ReplayDataProcessor &&) noexcept = default;

CryptoResult<void>
ReplayDataProcessor::set_encryption_key(std::span<const uint8_t> key) {
  return m_impl->decryptor.set_key(key);
}

CryptoResult<void>
ReplayDataProcessor::set_encryption_key(const std::vector<uint8_t> &key) {
  return set_encryption_key(std::span<const uint8_t>(key));
}

void ReplayDataProcessor::set_decompressor(
    std::unique_ptr<Decompressor> decompressor) {
...
CryptoResult<std::vector<uint8_t>>
ReplayDataProcessor::process(std::span<const uint8_t> data, bool is_encrypted,
                             bool is_compressed) {
  if (data.empty()) {
  ...
  if (is_encrypted) {
    ...
    if (!decrypt_result) {
      fprintf(stderr, "ERROR: Decryption failed: %s\n",
              crypto_error_string(decrypt_result.error()));
  ...
  if (is_compressed) {
    ...
    // Helper to read LE uint32_t from a vector and remove the bytes
    auto pop_uint32 = [](std::vector<uint8_t> &vec) -> uint32_t {
      if (vec.size() < 4)
        return 0;
      uint32_t value = static_cast<uint32_t>(vec[0]) |
                       (static_cast<uint32_t>(vec[1]) << 8) |
                       (static_cast<uint32_t>(vec[2]) << 16) |
                       (static_cast<uint32_t>(vec[3]) << 24);
      vec.erase(vec.begin(), vec.begin() + 4);
      return value;
    };

    uint32_t decompressed_size = pop_uint32(working_data);
    uint32_t compressed_size = pop_uint32(working_data);

    if (working_data.size() < compressed_size) {
      fprintf(stderr,
              "ERROR: Not enough data! Have %zu bytes but need %u bytes\n",
              working_data.size(), compressed_size);
      return std::unexpected(CryptoError::InvalidCompressedData);
    }
...
parse_compressed_chunk_header(std::span<const uint8_t> data) {
  ...
  uint32_t decompressed_size = static_cast<uint32_t>(data[0]) |
                               (static_cast<uint32_t>(data[1]) << 8) |
                               (static_cast<uint32_t>(data[2]) << 16) |
                               (static_cast<uint32_t>(data[3]) << 24);

  // Read compressed size (little-endian)
  uint32_t compressed_size = static_cast<uint32_t>(data[4]) |
