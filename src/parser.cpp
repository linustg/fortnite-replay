    ...
    Replay FortniteReplayParser::replay()
    {
        ...
        if (m_replay->chunks())
        {
            for (const auto *chunk : *m_replay->chunks())
            {
                ...
                {
                    auto *header_chunk = static_cast<fortnite_replay_t::header_chunk_t *>(chunk->chunk_data());
                    if (!header_chunk)
                        break;
                    ...
                    header.game_network_protocol_version = header_chunk->game_network_protocol_version();

                    if (!header_chunk->_is_null_guid())
                        header.guid = header_chunk->guid();

                    if (!header_chunk->_is_null_version_full() && header_chunk->version_full())
                    {
                        auto *ver = header_chunk->version_full();
                        header.engine_version = EngineVersion{
                            ver->major(),
                            ver->minor(),
                            ver->patch(),
                            ver->changelist(),
                            extract_fstring(ver->branch())};
                    }

                    if (!header_chunk->_is_null_recording_metadata() && header_chunk->recording_metadata())
                    {
                        auto *meta = header_chunk->recording_metadata();
                        header.recording_metadata = RecordingMetadata{
                            meta->ue4_version(),
                            meta->ue5_version(),
                            meta->package_version_licensee_ue()};
                    }

                    if (header_chunk->level_names_and_times() && header_chunk->level_names_and_times()->entries())
                    {
                        for (const auto *level : *header_chunk->level_names_and_times()->entries())
                        {
                            header.levels.push_back(LevelInfo{
                                extract_fstring(level->name()),
                                level->time()});
                        }
                    }
                    ...
                    if (header_chunk->game_specific_data() && header_chunk->game_specific_data()->entries())
                    {
                        for (const auto *str : *header_chunk->game_specific_data()->entries())
                        {
                            header.game_specific_data.push_back(extract_fstring(str));
                        }
                    }

                    if (!header_chunk->_is_null_package_version_info() && header_chunk->package_version_info())
                    {
                    ...
                    if (!header_chunk->_is_null_custom_versions() && header_chunk->custom_versions())
                    {
                ...
                {
                    auto *data_chunk = static_cast<fortnite_replay_t::replay_data_chunk_t *>(chunk->chunk_data());
                    if (!data_chunk)
                        break;
                ...
                {
                    auto *cp_chunk = static_cast<fortnite_replay_t::checkpoint_chunk_t *>(chunk->chunk_data());
                    if (!cp_chunk)
                        break;
                ...
                {
                    auto *event_chunk = static_cast<fortnite_replay_t::event_chunk_t *>(chunk->chunk_data());
                    if (!event_chunk)
                        break;
