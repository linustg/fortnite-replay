 ...
 */
template <typename T> using CryptoResult = std::expected<T, CryptoError>;
...
public:
  ...
  decrypt(std::span<const uint8_t> encrypted_data);

  /**
   * Static convenience function for one-shot decryption
   */
  static CryptoResult<std::vector<uint8_t>>
  decrypt(std::span<const uint8_t> key,
 ...
 *
 * Fortnite/UE replays use Oodle compression for replay data. Since Oodle is
 * proprietary, this provides an abstraction layer. The OodleDecompressor uses
 * the Oodle library from a Fortnite installation or other source.
 */
class Decompressor {
public:
   ...
   */
  virtual CryptoResult<std::vector<uint8_t>>
  decompress(std::span<const uint8_t> compressed_data,
   ...
   */
  virtual CryptoResult<size_t>
  decompress_to(std::span<const uint8_t> compressed_data,
                std::span<uint8_t> output, size_t decompressed_size) = 0;
};
...
public:
  ...
  CryptoResult<size_t> decompress_to(std::span<const uint8_t> compressed_data,
...
public:
   ...
   * @param decompressed_size Expected decompressed size (required if
   * compressed)
   * @return Processed data or error
   */
  CryptoResult<std::vector<uint8_t>>
  process(std::span<const uint8_t> data, bool is_encrypted, bool is_compressed);
