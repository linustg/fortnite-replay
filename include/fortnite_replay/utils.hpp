...
namespace fortnite_replay::Utils
{
    ...
    {
        if (type_str == event_type::PLAYER_ELIMINATION) return EventType::PlayerElimination;
        if (type_str == event_type::MATCH_STATS) return EventType::MatchStats;
        if (type_str == event_type::TEAM_STATS) return EventType::TeamStats;
        if (type_str == event_type::ENCRYPTION_KEY) return EventType::EncryptionKey;
        if (type_str == event_type::CHARACTER_SAMPLE) return EventType::CharacterSample;
        if (type_str == event_type::ZONE_UPDATE) return EventType::ZoneUpdate;
        if (type_str == event_type::BATTLE_BUS) return EventType::BattleBus;
